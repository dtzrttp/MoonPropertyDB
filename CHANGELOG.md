...
- On the unmerged `codex/6-property-equality-indexes` branch, added private
  ...
  set/remove, strict delete, and cascade delete. Edge postings are built at
  index creation but edge mutation maintenance is still pending. The latest
  native suite passes 21/21. This is a partial Issue #6 implementation, not a
  closable issue: edge lifecycle maintenance, public APIs/CLI, Issue #7 rollback
  atomicity, and persistence/snapshot integration remain outstanding.
...
- The Issue #13 base branch implements private in-memory node/edge state. The
  ...
  node lifecycle posting maintenance; edge lifecycle posting maintenance is
  not yet implemented. Public Database APIs/CLI, Issue #7 rollback atomicity,
  persistence, recovery, snapshots, and queries remain unimplemented or
  outstanding.
